...
public class ReservaVista extends javax.swing.JPanel implements ActionListener {
    ...
    private void jBcrearMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jBcrearMouseClicked
        ...
        if (siNombre && siDocumento && ValidacionDatos.validarDNI(jTdocumento.getText()) ) {
            ...
            List<Mesa> mesasSinReservas = md.obtenerTodasMesasSinReservas();

             
            if (mesasSinReservas.isEmpty()) {
                JOptionPane.showMessageDialog(this, "No hay mesas disponibles sin reservas.");
            } else {
                // Crear un JComboBox con las mesas sin reservas
                JComboBox<Mesa> mesaComboBox = new JComboBox<>(mesasSinReservas.toArray(new Mesa[0]));

                // Crear un cuadro de diálogo para mostrar el JComboBox
                int result = JOptionPane.showConfirmDialog(this, mesaComboBox, "Selecciona una mesa", JOptionPane.OK_CANCEL_OPTION);

                if (result == JOptionPane.OK_OPTION) {
                    Mesa mesaSeleccionada = (Mesa) mesaComboBox.getSelectedItem();
                    
                 //aca tengo la mesa que quiere el cliente
            ...
            rd.guardarReserva(rv);//creo la reserva
            md.agregarIdReserva(mesaSeleccionada.getIdMesa(), rv.getIdReserva());
